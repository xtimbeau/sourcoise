...
[sourcoise](https://xtimbeau.github.io/sourcoise/) (pronouced
\[suʁsɔiz\] as *sourçoïse*) is a package that provides tools for running
...
isolated, and that is improving reproducibility.
...
provides tools, for checking the cache,
[`sourcoise_status()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise_status.md),
and refreshing it on demand,
[`sourcoise_refresh()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise_refresh.md),
or at a given frequency through a parameter of
[`sourcoise()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise.md).

## Installation

[sourcoise](https://xtimbeau.github.io/sourcoise/) can be installed from
CRAN:
...
[`sourcoise_status()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise_status.md).
It will scan your project and collect info about your cached data. To
refresh everything, call
[`sourcoise_refresh()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise_refresh.md),
it will execute all scripts and refresh data in the cache. Options are
available to filter what really need to be refreshed.
...
1.  time saving when code execution takes a long time (accessing an API,
    ...
    is cached, thanks to optimization and
...
3.  if the source code triggers an error, you can override it: In the
    ...
    OECD API, particularly unreliable), then
    [`sourcoise()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise.md)
    tries to take the result of the last successful execution (if
    cached). Although this can be problematic, i.e. an unreported error
    and use of an ivalid cache, it has the enormous advantage of not
    blocking the process and allows to handle the error in parallel.

4.  [`sourcoise()`](https://xtimbeau.github.io/sourcoise/reference/sourcoise.md)
    cleverly searches for the source file in the project and executes
    the code in a local environment, changing the working directory to
    the one where the source code is located. This makes it possible to
    call the source code (the script `r` `mon_script.r` passed as a
    parameter to `sourcoise("mon_script.r")` scripts `r` data files
    `.csv` or `.xlsx` which are saved in the same directory as the
    `mon_script.r`. You can therefore reuse the code without having to
    worry about modifying the paths, which are relative to the folder
    where `mon_script.r` is. Then, the code is resusable anywhere in the
    project or elsewhere. Note that it is done with simple copy of
    source scripts and does not compare to a common code called in many
    situation, which involves another approach (a package and functions
    for instance).
...
- the ability to store hidden data outside of the project folder (and
  therefore outside of *github*) and to use
  [pins](https://pins.rstudio.com/) for storage (but perhaps at the cost
  of slower access).
