...
checking the cache, `sourcoise_status()`, and refreshing it on demand, `sourcoise_refresh()`, or at a given frequency trhough a parameter of `sourcoise()`.
...
The first time the script is run completly, subsequent calls will use the cache on disk, persistant across session, unless the cache is invalidated.
