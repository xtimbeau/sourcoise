...
It also means that the script code that retrieves the data from a file has to be isolated to improve reproducibility. `sourcoise()` can be called in a `sourcoise()` which allows modularity. It provides tools, for
checking the cache, `sourcoise_status()`, and refreshing it, `sourcoise_refresh()`, on demand.
